...
const TradeCalls = {
  ...
  create: async (data) => {
    const [result] = await db.execute(
      `INSERT INTO trade_calls
        (stock_id, call_type, recommended_price, target_price, stop_loss, lots, description, is_active, created_at, updated_at)
       VALUES (?,?,?,?,?,?,?,1,NOW(),NOW())`,
      [data.stock_id, data.call_type || 'BUY', data.recommended_price,
       data.target_price || null, data.stop_loss || null,
  ...
  getAllByPage: async (limit, pageNo, searchtxt, isActive) => {
    ...
    let query = `SELECT tc.*, s.name AS stock_name, s.symbol,
    ...
    if (searchtxt) { countQ += ' AND (s.name LIKE ? OR s.symbol LIKE ?)'; countP.push(`%${searchtxt}%`, `%${searchtxt}%`); }
  ...
  getById: async (id) => {
    const [rows] = await db.execute(
      `SELECT tc.*, s.name AS stock_name, s.symbol, s.lot_size
  ...
  update: async (id, data) => {
    await db.execute(
      `UPDATE trade_calls SET stock_id=?, call_type=?, recommended_price=?, target_price=?,
       stop_loss=?, lots=?, description=?, updated_at=NOW() WHERE id=?`,
