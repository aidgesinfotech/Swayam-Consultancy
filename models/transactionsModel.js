...
const Transactions = {
  ...
  getAllByClient: async (clientId, limit, pageNo, type) => {
    ...
    let query = `SELECT t.*, s.name AS stock_name, s.symbol, s.lot_size, s.current_price
                 FROM transactions t
  ...
  getAllByPage: async (limit, pageNo, searchtxt, clientId) => {
    const offset = (pageNo - 1) * limit;
    let query = `SELECT t.*, s.name AS stock_name, s.symbol, c.name AS client_name
                 FROM transactions t
                 JOIN stocks s ON t.stock_id = s.id
    ...
    if (searchtxt) { query += ' AND (s.name LIKE ? OR s.symbol LIKE ? OR c.name LIKE ?)'; params.push(`%${searchtxt}%`, `%${searchtxt}%`, `%${searchtxt}%`); }
    query += ' ORDER BY t.created_at DESC LIMIT ? OFFSET ?';
    params.push(limit, offset);
    const [results] = await db.execute(query, params);
    ...
    if (searchtxt) { countQuery += ' AND (s.name LIKE ? OR s.symbol LIKE ? OR c.name LIKE ?)'; countParams.push(`%${searchtxt}%`, `%${searchtxt}%`, `%${searchtxt}%`); }
    const [countResult] = await db.execute(countQuery, countParams);
  ...
  getById: async (id) => {
    const [results] = await db.execute(
      `SELECT t.*, s.name AS stock_name, s.symbol, s.lot_size, s.current_price, c.name AS client_name
       FROM transactions t
       JOIN stocks s ON t.stock_id = s.id
       JOIN clients c ON t.client_id = c.id
       WHERE t.id = ?`, [id]
  ...
  getPortfolioByClient: async (clientId) => {
    // Aggregate open buy positions per stock with P&L
    const [results] = await db.execute(
      `SELECT t.stock_id, s.name AS stock_name, s.symbol, s.lot_size, s.current_price, s.sector,
       ...
       JOIN stocks s ON t.stock_id = s.id
       WHERE t.client_id = ?
       GROUP BY t.stock_id, s.name, s.symbol, s.lot_size, s.current_price, s.sector
