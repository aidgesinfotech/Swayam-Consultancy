...
const TradeBookings = {
  ...
  getMyBookings: async (clientId, status) => {
    let query = `SELECT tb.*,
                        ...
                        s.name AS stock_name, s.lot_size, s.qty, s.expiry_date, s.strike_price
  ...
  getAllAdmin: async (limit, pageNo, clientId, status, search) => {
    ...
    if (status)   { query += ' AND tb.status = ?';    params.push(status); }
    if (search)   { query += ' AND (c.name LIKE ? OR s.name LIKE ? OR c.email LIKE ?)'; params.push(`%${search}%`, `%${search}%`, `%${search}%`); }
    query += ' ORDER BY tb.created_at DESC LIMIT ? OFFSET ?';
    params.push(limit, offset);
    const [results] = await db.execute(query, params);

    let countQ = `SELECT COUNT(*) AS totalCount FROM trade_bookings tb
                  JOIN trade_calls tc ON tb.call_id = tc.id
                  JOIN stocks s ON tc.stock_id = s.id
                  JOIN clients c ON tb.client_id = c.id WHERE 1=1`;
    const countP = [];
    if (clientId) { countQ += ' AND tb.client_id = ?'; countP.push(clientId); }
    if (status)   { countQ += ' AND tb.status = ?';    countP.push(status); }
    if (search)   { countQ += ' AND (c.name LIKE ? OR s.name LIKE ? OR c.email LIKE ?)'; countP.push(`%${search}%`, `%${search}%`, `%${search}%`); }
