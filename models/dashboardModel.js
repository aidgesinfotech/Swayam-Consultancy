  ...
  adminDashboard: async () => {
    try {
      ...
      const [topCalls] = await db.execute(`
        ...
        GROUP BY tc.id, tc.call_type, tc.recommended_price, tc.is_active, s.name, s.symbol
      ...
      const [sectorDist] = await db.execute(`
        SELECT s.sector,
          COUNT(tb.id) AS bookings,
          COALESCE(SUM(tb.booking_price * tb.quantity), 0) AS exposure
        FROM trade_bookings tb
        JOIN trade_calls tc ON tb.call_id = tc.id
        JOIN stocks s ON tc.stock_id = s.id
        WHERE tb.status = 'open' AND s.sector IS NOT NULL
        GROUP BY s.sector
