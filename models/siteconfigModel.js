...
const siteconfig = {
    create: async (data) => {
        const sql = 'INSERT INTO siteconfig (siteName,clientUrl, logo, whiteLogo, icon, instagramURL, facebookURL, twitterURL, linkedInURL, youtubeURL, mobile, email, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())';
        try {
            const [results] = await db.execute(sql, [data.siteName, data.clientUrl, data.logo, data.whiteLogo, data.icon, data.instagramURL, data.facebookURL, data.twitterURL,data.linkedInURL, data.youtubeURL, data.mobile,data.email]);
    ...
    update: async (id, data) => {
        ...
        try {
            const [results] = await db.execute(sqlUpdate, [
                ...
                data.mobile, data.email, data.primaryColor || '#2563eb',
                id
            ]);
